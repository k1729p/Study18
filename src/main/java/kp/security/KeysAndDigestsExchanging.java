...
public class KeysAndDigestsExchanging {
	...
	private static void showKeysInBase64(KeyPair keyPair) {
		...
		Printer.printf("private key changed to Base64 (length[%3d]):%n [%s]", base64Priv.length(), base64Priv);
		Printer.printf("public  key changed to Base64 (length[%3d]):%n [%s]", base64Pub.length(), base64Pub);
	}
}
